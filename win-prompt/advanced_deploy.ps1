...
$PROJECT_ID = "win-prompt-mandeepkaur"
...
if ($LASTEXITCODE -ne 0) {
    ...
    & $Gcloud artifacts repositories create $REPOSITORY --repository-format=docker --location=$REGION --description="Docker repo for TruthLens" --project=$PROJECT_ID
} else {
...
& $Gcloud run deploy $SERVICE_NAME --image $IMAGE_TAG --platform managed --region $REGION --allow-unauthenticated --port 8080 --set-env-vars "GEMINI_API_KEY=AIzaSyAdpSU3F5gdt3zFULf1_BSUIDjXUSbllSk,GOOGLE_SEARCH_API_KEY=AIzaSyAdpSU3F5gdt3zFULf1_BSUIDjXUSbllSk,GOOGLE_SEARCH_CX=d4a6704242f7b4cfe" --project=$PROJECT_ID
